...
SAFE_DOMAINS=('greenhouse.io','lever.co','smartrecruiters.com','workable.com','workdayjobs.com','myworkdayjobs.com')
BLOCKED_DOMAINS=('linkedin.com','indeed.com','bayt.com','gulftalent.com','naukrigulf.com')

KNOWN={
 'first name':FIRST,'firstname':FIRST,'last name':LAST,'lastname':LAST,
 'email':EMAIL,'phone':PHONE,'mobile':PHONE,'location':LOCATION,
 'linkedin':LINKEDIN,'website':PORTFOLIO,'portfolio':PORTFOLIO,
}
...
def fill_known(page):
    # text-like fields only; never guesses screening answers
    ...
    # common name attrs/placeholders
    selectors={
      'input[name*="first" i]':FIRST,'input[name*="last" i]':LAST,
      'input[type="email"]':EMAIL,'input[type="tel"]':PHONE,
      'input[name*="linkedin" i]':LINKEDIN,'input[name*="website" i]':PORTFOLIO,
    }
...
def try_apply(page,url,row):
    ...
    if any(d in h for d in BLOCKED_DOMAINS):
        set_status(row,'Needs Manual Submission','Job board requires authenticated/manual submission or blocks unattended automation.','Manual job-board apply')
        return
    if not any(d in h for d in SAFE_DOMAINS):
        set_status(row,'Needs Manual Submission','No safe supported ATS adapter detected for this vacancy URL.','Manual external ATS')
        return
    ...
    try:
        ...
        # enter application form if a visible apply button exists
        ...
        if missing:
            ...
            return
        # final submit: only after required known fields are satisfied
        submitted=False
        for pat in (r'submit application',r'submit',r'apply now'):
            try:
                btn=page.get_by_role('button',name=re.compile(pat,re.I)).last
                if btn.is_visible() and btn.is_enabled():
                    btn.click(); page.wait_for_timeout(3000); submitted=True; break
            except: pass
...
with sync_playwright() as p:
    ...
    for row in queue:
        try_apply(page,row.get('job_url',''),row)
    browser.close()
data['updated_at']=now()
